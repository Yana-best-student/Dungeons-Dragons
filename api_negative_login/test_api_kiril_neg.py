...
@allure.title("Регистрация с логином, содержащим кириллицу")
@allure.description(
    "Тест проверяет реакцию системы на попытку регистрации с логином, "
    "содержащим кириллические символы."
)
def test_invalid_login_with_cyrillic_chars():
    """
    Тест регистрирует нового пользователя с логином, содержащим кириллицу.
    Ожидается отказ регистрации.
    """

    # Логин длиной 20 символов с кириллицей
    invalid_username = "приветмиркириллица"
    ...
    # Исправленная часть кода для проверки сообщения об ошибке
