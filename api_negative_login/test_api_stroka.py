...
@allure.title("Регистрация с пустой строкой в поле логина")
@allure.description(
    "Тест проверяет реакцию системы на попытку регистрации с пустой строкой в поле логина. "
    "Ожидается отказ регистрации."
)
def test_invalid_login_empty_string():
    """
    Тест регистрирует нового пользователя с пустой строкой в поле логина.
    Ожидается отказ регистрации.
    """
    ...
    payload = {
        'username': empty_username,  # Пустой логин
    ...
    with allure.step("Отправить запрос на регистрацию с пустым логином"):
    ...
    # Гибкая проверка наличия ключевой фразы в сообщении об ошибке
    assert "cannot be blank" in response_json["username"][0], \
           f"Ожидалось сообщение, содержащее фразу 'cannot be blank', но получено: {response_json['username'][0]}"

    # Прикрепляем сообщение об ошибке к отчёту
    allure.attach(response_json["username"][0], name="Сообщение об ошибке", attachment_type=allure.attachment_type.TEXT)