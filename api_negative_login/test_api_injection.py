...
@allure.title("Регистрация с логином, содержащим XSS-вектор длиной 30 символов")
@allure.description(
    "Тест проверяет реакцию системы на попытку регистрации с логином длиной 30 символов, "
    "включающим XSS-вектор '<script>alert(1)</script>'."
)
def test_invalid_login_xss_vector_30_chars():
    """
    Тест регистрирует нового пользователя с логином длиной 30 символов, включающим XSS-вектор.
    Ожидается отказ регистрации.
    """

    # Логин длиной ровно 30 символов, содержащий XSS-вектор
    invalid_username = "<script>alert(1)</scrip>"  # длина ровно 30 символов
    ...
    with allure.step("Проверить сообщение об ошибке"):
        ...
        assert isinstance(
            response_json["username"], list), "Поле 'username' не является массивом"
