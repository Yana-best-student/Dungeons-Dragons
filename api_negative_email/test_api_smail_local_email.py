...
@allure.title("Регистрация email с эмодзи в локальной части")
@allure.description("""
Тест проверяет реакцию системы на попытку регистрации с email, содержащим эмодзи в локальной части.
Ожидается отказ регистрации с соответствующим сообщением об ошибке.
""")
@allure.tag("Negative", "EmailValidation", "Registration")
def test_invalid_local_email_with_emoji():
    """
    Негативный тест регистрации с email, содержащим эмодзи в локальной части.
    Ожидается отказ регистрации с сообщением об ошибке.
    """

    # Недопустимый email (эмодзи в локальной части)
    invalid_email = "ivan😚petrov@mail.ru"
