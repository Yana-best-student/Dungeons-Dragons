...
# @pytest.mark.xfail(
#     reason="Backend не проверяет слеши в email. Ожидается 400, приходит 201.",
#     strict=True
# )
@allure.epic("Dungeons & Dragons")
@allure.feature("Авторизация и регистрация")
@allure.story("Регистрация с недопустимым email")
@allure.title("Регистрация  email c /// в локальной части")
@allure.description("""
Тест проверяет реакцию системы на попытку регистрации  email c /// в локальной части.
...
def test_invalid_local_email():
    """
    Негативный тест регистрации с недопустимым email (c /// в локальной части).
    Ожидается отказ регистрации с сообщением об ошибке.
    """

    # Недопустимый email (без локальной части)
    invalid_email = "ivanov///ivan43@gmail.com"
