...
@allure.title("Регистрация  содержащая XSS уязвимость в поле email")
@allure.description("""
Тест проверяет реакцию системы на попытку регистрации  email c XSS уязвимостью.
...
def test_invalid_local_email():
    """
    Негативный тест регистрации с недопустимым email (c XSS уязвимостью).
    Ожидается отказ регистрации с сообщением об ошибке.
    """

    # Недопустимый email (без локальной части)
    invalid_email = "<script>alert(1)</script>@gmail.com"
